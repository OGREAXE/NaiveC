...
@implementation NPMethodSignature {
    NSString *_typeNames;
...
- (void)_parseTypeNames
{
    ...
    for (NSInteger i = 0; i < typeArr.count; i++) {
        ...
        NSString *encode = [NPMethodSignature typeEncodeWithTypeName:typeStr];
        if (!encode) {
        ...
        int length = [NPMethodSignature typeLengthWithTypeName:typeStr];
    ...
    switch (c[0]) {
        ...
        case '{':
        {
            ...
            if (end != NSNotFound) {
                ...
                NSDictionary *structDefine = [NPEngine registeredStruct][structName];
